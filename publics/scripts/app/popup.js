		...
		Coinmarketcap.getList(function (coinmarketcapInfos) {
			...
			if(infos.connected == true) {
				...
				for(var id in infos.wallets) {
					...
					// Get coin infos
					var coinInfos = Coinmarketcap.getFromListWithSymb(coinmarketcapInfos.data, infos.wallets[id].symb);

					// Append element
					$("body > .content > .wallets > .content").append('<div class="wallet"><img src="'+config.website.hostUrl+'/images/icons/'+infos.wallets[id].symb+'.svg" class="icon" /><span class="value">'+infos.wallets[id].value+'</span> <span class="usd" title="'+Lang.get("content-wallets-wallet-title")+'">( '+(Math.round((infos.wallets[id].value * coinInfos.quotes.USD.price) * 100) / 100)+'$ )</span> <span class="symb">'+infos.wallets[id].symb+'</span></div>');
				}
				// Load contracts list
